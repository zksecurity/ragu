...
namespace Ragu.Instances.Autogen.Element.IsEqual
open Core.Primes
...
def exportedOperations (input_var : Vector (Expression (F p)) inputLen) : Operations (F p) := [
  ...
  Operation.assert (((var ⟨0⟩) + (((-1 : F p) : Expression (F p)) * ((input_var[0]) + (((-1 : F p) : Expression (F p)) * (input_var[1])))))),
  Operation.assert ((var ⟨2⟩)),
  Operation.witness 3 (fun _env => default),
  Operation.assert ((((var ⟨3⟩) * (var ⟨4⟩)) + (((-1 : F p) : Expression (F p)) * (var ⟨5⟩)))),
  Operation.assert (((var ⟨3⟩) + (((-1 : F p) : Expression (F p)) * ((input_var[0]) + (((-1 : F p) : Expression (F p)) * (input_var[1])))))),
...
end Ragu.Instances.Autogen.Element.IsEqual
